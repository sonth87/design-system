...
export function AdminLayout({ children }: AdminLayoutProps) {
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        {/* Header */}
        <header className="ds:sticky ds:top-0 ds:z-10 ds:border-b ds:bg-background/95 ds:backdrop-blur supports-[backdrop-filter]:bg-background/60">
                  ...
                  <div className="w-[320px]">
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                      {notifications.map((notification) => (
                        <div
                          key={notification.id}
                          className={`ds:flex ds:gap-3 ds:px-4 ds:py-3 ds:border-b ds:last:border-0 ds:hover:bg-accent/50 ds:cursor-pointer ds:transition-colors ${ notification.unread ? "ds:bg-accent/20" : "" }`}
                        >
                          <div
                            className={`ds:flex ds:h-10 ds:w-10 ds:flex-shrink-0 ds:items-center ds:justify-center ds:rounded-full ${ notification.type === "success" ? "ds:bg-green-100 ds:text-green-600" : notification.type === "warning" ? "ds:bg-yellow-100 ds:text-yellow-600" : "ds:bg-blue-100 ds:text-blue-600" }`}
                          >
        ...
        {/* Page Content */}
        <main className="ds:flex-1 overflow-auto ds:p-6">{children}</main>
      </SidebarInset>
    </SidebarProvider>
  );
}
