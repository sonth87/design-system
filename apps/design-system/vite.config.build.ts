...
function getEntryPoints() {
  ...
  if (mode === "ADVANCED") {
    console.log("📦 Building in ADVANCED mode - creating individual component entries");
    ...
    componentDirs.forEach((dir) => {
      ...
      const componentPath = path.resolve(__dirname, dir, `${componentName}.tsx`);
      
      // Check if component file exists
      if (fs.existsSync(componentPath)) {
        // Use lowercase for consistency: button, avatar, etc.
        entries[`components/${componentName.toLowerCase()}/index`] = componentPath;
      }
    });
    ...
    standaloneFiles.forEach((file) => {
      ...
      entries[`components/${fileName.toLowerCase()}`] = path.resolve(__dirname, file);
    });
    
    console.log(`✅ Found ${Object.keys(entries).length - 1 - cssFiles.length} components`);
  } else {
    console.log("📦 Building in SIMPLE mode - only src/index.ts (components exported there will be included)");
  }
