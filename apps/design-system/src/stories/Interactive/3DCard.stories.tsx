import type { Meta, StoryObj } from "@storybook/react";
import {
  CardContainer,
  CardBody,
  CardItem,
} from "../../components/Interactive/3DCard";
import i18n from "../../../.storybook/i18n";

const meta: Meta<typeof CardContainer | typeof CardBody | typeof CardItem> = {
  title: "Interactive/3DCard",
  component: CardContainer,
  parameters: {
    layout: "centered",
  },
  tags: ["autodocs"],
  argTypes: {
    // CardContainer Props
    containerClassName: {
      control: "text",
      description: i18n.t(
        "stories.3dcard.argTypes.containerClassName.description"
      ),
      table: {
        category: "CardContainer",
      },
    },
    perspective: {
      control: { type: "number", min: 500, max: 2000 },
      description: i18n.t("stories.3dcard.argTypes.perspective.description"),
      table: {
        defaultValue: { summary: "1000" },
        category: "CardContainer",
      },
    },
    rotationPreset: {
      control: { type: "select" },
      options: ["low", "medium", "high"],
      description: i18n.t("stories.3dcard.argTypes.rotationPreset.description"),
      table: {
        defaultValue: { summary: "medium" },
        category: "CardContainer",
      },
    },

    // CardBody Props
    className: {
      control: "text",
      description: i18n.t("stories.3dcard.argTypes.className.description"),
      table: {
        category: "CardBody",
      },
    },

    // CardItem Props
    as: {
      control: "text",
      description: i18n.t("stories.3dcard.argTypes.as.description"),
      table: {
        defaultValue: { summary: "div" },
        category: "CardItem",
      },
    },
    children: {
      control: false,
      description: i18n.t("stories.3dcard.argTypes.children.description"),
      table: {
        category: "CardItem",
      },
    },

    // Transform Props - Translation
    translateX: {
      control: { type: "number" },
      description: i18n.t("stories.3dcard.argTypes.translateX.description"),
      table: {
        defaultValue: { summary: "0" },
        category: "CardItem - Transform",
      },
    },
    translateY: {
      control: { type: "number" },
      description: i18n.t("stories.3dcard.argTypes.translateY.description"),
      table: {
        defaultValue: { summary: "0" },
        category: "CardItem - Transform",
      },
    },
    translateZ: {
      control: { type: "number" },
      description: i18n.t("stories.3dcard.argTypes.translateZ.description"),
      table: {
        defaultValue: { summary: "0" },
        category: "CardItem - Transform",
      },
    },

    // Transform Props - Rotation
    rotateX: {
      control: { type: "number" },
      description: i18n.t("stories.3dcard.argTypes.rotateX.description"),
      table: {
        defaultValue: { summary: "0" },
        category: "CardItem - Transform",
      },
    },
    rotateY: {
      control: { type: "number" },
      description: i18n.t("stories.3dcard.argTypes.rotateY.description"),
      table: {
        defaultValue: { summary: "0" },
        category: "CardItem - Transform",
      },
    },
    rotateZ: {
      control: { type: "number" },
      description: i18n.t("stories.3dcard.argTypes.rotateZ.description"),
      table: {
        defaultValue: { summary: "0" },
        category: "CardItem - Transform",
      },
    },
  },
};

export default meta;
type Story = StoryObj<typeof CardContainer | typeof CardBody | typeof CardItem>;

export const Default: Story = {
  render: () => (
    <CardContainer className="ds:inter-var" containerClassName="ds:py-8">
      <CardBody className="ds:bg-gray-50 ds:relative ds:group/card ds:dark:hover:shadow-2xl ds:dark:hover:shadow-emerald-500/10 ds:dark:bg-black ds:dark:border-white/20 ds:border-black/10 ds:w-auto ds:sm:w-[24rem] ds:h-auto ds:rounded-xl ds:border ds:p-4">
        <CardItem
          translateZ="50"
          className="ds:text-xl ds:font-bold ds:text-neutral-600 ds:dark:text-white"
        >
          Make things float in air
        </CardItem>
        <CardItem
          as="p"
          translateZ="60"
          className="ds:text-neutral-500 ds:text-sm ds:max-w-sm ds:mt-2 ds:dark:text-neutral-300"
        >
          Hover over this card to unleash the power of CSS perspective
        </CardItem>
        <CardItem translateZ="100" className="ds:w-full ds:mt-4">
          <img
            src="https://images.unsplash.com/photo-1441974231531-c6227db76b6e?q=80&w=2560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            height="1000"
            width="1000"
            className="ds:h-48 ds:w-full ds:object-cover ds:rounded-xl ds:group-hover/card:shadow-xl"
            alt="thumbnail"
          />
        </CardItem>
        <div className="ds:flex ds:justify-between ds:items-center ds:mt-16">
          <CardItem
            translateZ={20}
            as="a"
            href="https://twitter.com/mannupaaji"
            target="__blank"
            className="ds:px-4 ds:py-2 ds:rounded-xl ds:text-xs ds:font-normal ds:dark:text-white"
          >
            Try now →
          </CardItem>
          <CardItem
            translateZ={20}
            as="button"
            className="ds:px-4 ds:py-2 ds:rounded-xl ds:bg-black ds:dark:bg-white ds:dark:text-black ds:text-white ds:text-xs ds:font-bold"
          >
            Sign up
          </CardItem>
        </div>
      </CardBody>
    </CardContainer>
  ),
};

export const ComplexExample: Story = {
  render: () => (
    <CardContainer className="ds:inter-var" containerClassName="ds:py-8">
      <CardBody className="ds:bg-gray-50 ds:relative ds:group/card ds:dark:hover:shadow-2xl ds:dark:hover:shadow-emerald-500/10 ds:dark:bg-black ds:dark:border-white/20 ds:border-black/10 ds:w-auto ds:sm:w-[24rem] ds:h-auto ds:rounded-xl ds:p-4 ds:border">
        <CardItem
          translateZ="50"
          className="ds:text-xl ds:font-bold ds:text-neutral-600 ds:dark:text-white"
        >
          Interactive 3D Card
        </CardItem>
        <CardItem
          as="p"
          translateZ="60"
          className="ds:text-neutral-500 ds:text-sm ds:max-w-sm ds:mt-2 ds:dark:text-neutral-300"
        >
          Experience smooth transforms with enhanced rotations and translations
        </CardItem>
        <CardItem
          translateZ="100"
          rotateX={20}
          rotateZ={-10}
          className="ds:w-full ds:mt-4"
        >
          <img
            src="https://images.unsplash.com/photo-1441974231531-c6227db76b6e?q=80&w=2560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            height="1000"
            width="1000"
            className="ds:h-48 ds:w-full ds:object-cover ds:rounded-xl ds:group-hover/card:shadow-xl"
            alt="thumbnail"
          />
        </CardItem>
        <div className="ds:flex ds:justify-between ds:items-center ds:mt-16">
          <CardItem
            translateZ={20}
            translateX={-40}
            as="button"
            className="ds:px-4 ds:py-2 ds:rounded-xl ds:text-xs ds:font-normal ds:dark:text-white"
          >
            Try now →
          </CardItem>
          <CardItem
            translateZ={20}
            translateX={40}
            as="button"
            className="ds:px-4 ds:py-2 ds:rounded-xl ds:bg-black ds:dark:bg-white ds:dark:text-black ds:text-white ds:text-xs ds:font-bold"
          >
            Sign up
          </CardItem>
        </div>
      </CardBody>
    </CardContainer>
  ),
};
