...
export const CustomGroupStyles = () => {
  ...
  return (
    <Radio.Group
      className="ds:w-full ds:max-w-96 ds:min-w-80 ds:gap-0 ds:-space-y-px ds:rounded-md ds:shadow-xs"
      defaultValue="2"
    >
      {items.map((item, idx) => (
        <div
          key={`${idx}-${item.value}`}
          className="ds:border-input ds:has-data-[state=checked]:border-primary/50 ds:has-data-[state=checked]:bg-accent ds:relative ds:flex ds:flex-col ds:gap-4 ds:border ds:p-4 ds:outline-none ds:first:rounded-t-md ds:last:rounded-b-md ds:has-data-[state=checked]:z-10"
        >
          <div className="ds:flex ds:items-center ds:justify-between">
            ...
            <div
              ...
              className="ds:text-muted-foreground ds:text-xs ds:leading-[inherit]"
            >
              {item.price}
            </div>
          </div>
    ...
    <fieldset className="ds:w-full ds:space-y-4">
      ...
      <Radio.Group className="ds:grid ds:grid-cols-6 ds:gap-2" defaultValue="1">
        {items.map((item, idx) => (
          <label
            key={`${idx}-${item.value}`}
            className="ds:border-input ds:has-data-[state=checked]:border-secondary/80 ds:has-data-[state=checked]:bg-secondary/20 ds:has-focus-visible:border-ring ds:has-focus-visible:ring-ring/50 ds:relative ds:flex ds:flex-col ds:items-center ds:gap-2 ds:rounded-md ds:border ds:px-2 ds:py-3 ds:text-center ds:shadow-xs ds:transition-[color,box-shadow] ds:outline-none ds:has-focus-visible:ring-[3px] ds:has-data-disabled:cursor-not-allowed ds:has-data-disabled:opacity-50"
          >
            <Radio.Item
              id={`${idx}-${item.value}`}
              value={item.value}
              rootClassName="ds:sr-only ds:after:absolute ds:after:inset-0"
