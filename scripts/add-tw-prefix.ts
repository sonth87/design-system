...
const tailwindPattern = /\b[a-z-]+(?:\/[a-z0-9.-]+)?(?::[a-z-]+)?(?:\[[^\]]*\])?/g;

// List of Tailwind utilities to prefix
...
const excludeClasses = new Set([
  ...
  'focus-visible:ring', // This is partial - needs special handling
  // Add more custom classes as found
]);

/**
 * Checks if a string starts with a Tailwind modifier
 */
function hasTailwindModifier(str: string): boolean {
  return /^(hover|focus|focus-visible|active|disabled|group|sm|md|lg|xl|2xl|dark|light|before|after|first|last|data|even|odd):/.test(str);
}
...
async function processFiles() {
  const patterns = [
    ...
    'apps/design-system/src/**/*.{ts,js}',
    'packages/ui/src/**/*.{tsx,jsx}',
    'packages/ui/src/**/*.{ts,js}',
    'apps/docs/**/*.{tsx,jsx}',
    'apps/docs/**/*.{ts,js}',
  ];
  ...
  for (const pattern of patterns) {
    const files = await glob(pattern, { 
      ignore: ['**/node_modules/**', '**/.next/**', '**/dist/**', '**/*.stories.tsx']
    });
